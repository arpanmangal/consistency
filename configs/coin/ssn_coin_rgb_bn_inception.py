...
data_root = 'data/coin/subset_frames/'
ann_file_root = 'data/coin/'
img_norm_cfg = dict(
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=(ann_file_root + 'coin_tag_train_proposal_list.txt'),
    ...
    val=dict(
        type=dataset_type,
        ann_file=(ann_file_root + 'coin_tag_test_proposal_list.txt'),
        img_prefix=data_root,
        img_norm_cfg=img_norm_cfg,
        train_cfg=train_cfg,
        test_cfg=test_cfg,
    ...
    test=dict(
        ...
        test_mode=True))
# optimizer
optimizer = dict(type='SGD', lr=0.00025, momentum=0.9, weight_decay=1e-6)
...
log_level = 'INFO'
work_dir = './work_dirs/ssn_coin_2d_rgb_bn_inception'
load_from = None
resume_from = None
workflow = [('train', 1)]
